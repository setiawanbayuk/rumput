...
class SkboroController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('skboro.index', compact('title', 'rt', 'rw', 'kelurahan'));
    }
    ...
    public function warga()
    {
        ...
        $items = SuratBoro::query()
            ->where('nik', $nik)
            ->when($q, function ($query) use ($q) {
                $query->where(function ($w) use ($q) {
                    $w->where('no_urut_surat', 'like', "%{$q}%")
                        ->orWhere('peruntukan',   'like', "%{$q}%")
                        ->orWhere('prov_boro_nm',       'like', "%{$q}%")
                        ->orWhere('kabko_boro_nm',       'like', "%{$q}%")
                        ->orWhere('kec_boro_nm',       'like', "%{$q}%")
                        ->orWhere('kel_boro_nm',       'like', "%{$q}%")
                        ->orWhere('alamat_boro',       'like', "%{$q}%");
                });
            })
        ...
        return view('skboro.warga', compact(
            'title',
            'nama',
            'nik',
            'surat',
