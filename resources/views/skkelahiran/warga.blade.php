        ...
        <div class="tab-content mt-3">
            <div class="tab-pane fade show active" id="tab-detail" role="tabpanel">
                ...
                <div class="text-center">
                    <a href="{{ route('skkelahiran.create') }}" class="btn-ajukan">
                        Ajukan
                    </a>
                </div>
            </div>
                ...
                <div class="container rounded-3 px-4 py-2" style="background:rgba(174,160,122,.25)">
                    @forelse($sedangProses as $pengajuan)
                        @php
                            ...
                            $alias  = $pengajuan->jenisSurat ?? 'skkelahiran';
            ...
            {{-- Form global untuk hapus Skkelahiran (akan diisi otomatis oleh komponen) --}}
            <form id="formHapusSkkelahiran"
                method="POST"
                data-action-template="{{ route('skkelahiran.hapus', ['id' => '__ID__']) }}">
                @csrf
                {{-- hidden "id" akan dibuat/diisi otomatis oleh komponen saat modal dibuka --}}
            </form>
            <x-btn-hapus
                modalId="modalHapusSkkelahiran"
                formId="formHapusSkkelahiran"
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                const STORAGE_KEY = 'lastOpenedTabSkkelahiran';
        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                const modalEl = document.getElementById('modalHapusSkkelahiran');
                const modal   = new bootstrap.Modal(modalEl);
                const form    = document.getElementById('formHapusSkkelahiran');
                const submit  = document.getElementById('btnHapusSkkelahiran');
