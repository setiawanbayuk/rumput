        ...
        <div class="tab-content mt-3">
            <div class="tab-pane fade show active" id="tab-detail" role="tabpanel">
                ...
                <div class="text-center">
                    <a href="{{ route('skhsl.create') }}" class="btn-ajukan">
                        Ajukan
                    </a>
                </div>
            </div>
                ...
                <div class="container rounded-3 px-4 py-2" style="background:rgba(174,160,122,.25)">
                    @forelse($sedangProses as $pengajuan)
                        @php
                            ...
                            $alias  = $pengajuan->jenisSurat ?? 'skhsl';
            ...
            {{-- Form global untuk hapus Skhsl (akan diisi otomatis oleh komponen) --}}
            <form id="formHapusSkhsl"
                method="POST"
                data-action-template="{{ route('skhsl.hapus', ['id' => '__ID__']) }}">
                @csrf
                {{-- hidden "id" akan dibuat/diisi otomatis oleh komponen saat modal dibuka --}}
            </form>
            <x-btn-hapus
                modalId="modalHapusSkhsl"
                formId="formHapusSkhsl"
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                const STORAGE_KEY = 'lastOpenedTabSkhsl';
        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                const modalEl = document.getElementById('modalHapusSkhsl');
                const modal   = new bootstrap.Modal(modalEl);
                const form    = document.getElementById('formHapusSkhsl');
                const submit  = document.getElementById('btnHapusSkhsl');
