        ...
        <div class="tab-content mt-3">
            <div class="tab-pane fade show active" id="tab-detail" role="tabpanel">
                ...
                <div class="text-center">
                    <a href="{{ route('sktm.create') }}" class="btn-ajukan">
                        Ajukan
                    </a>
                </div>
            </div>
                ...
                <div class="container rounded-3 px-4 py-2" style="background:rgba(174,160,122,.25)">
                    @forelse($sedangProses as $pengajuan)
                        @php
                            ...
                            $alias  = $pengajuan->jenisSurat ?? 'sktm';
            ...
            {{-- Form global untuk hapus Sktm (akan diisi otomatis oleh komponen) --}}
            <form id="formHapusSktm"
                method="POST"
                data-action-template="{{ route('sktm.hapus', ['id' => '__ID__']) }}">
                @csrf
                {{-- hidden "id" akan dibuat/diisi otomatis oleh komponen saat modal dibuka --}}
            </form>
            <x-btn-hapus
                modalId="modalHapusSktm"
                formId="formHapusSktm"
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                const STORAGE_KEY = 'lastOpenedTabSktm';
        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                const modalEl = document.getElementById('modalHapusSktm');
                const modal   = new bootstrap.Modal(modalEl);
                const form    = document.getElementById('formHapusSktm');
                const submit  = document.getElementById('btnHapusSktm');
