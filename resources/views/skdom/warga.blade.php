        ...
        <div class="tab-content mt-3">
            <div class="tab-pane fade show active" id="tab-detail" role="tabpanel">
                ...
                <div class="text-center">
                    <a href="{{ route('skdom.create') }}" class="btn-ajukan">
                        Ajukan
                    </a>
                </div>
            </div>
                ...
                <div class="container rounded-3 px-4 py-2" style="background:rgba(174,160,122,.25)">
                    @forelse($sedangProses as $pengajuan)
                        @php
                            ...
                            $alias  = $pengajuan->jenisSurat ?? 'skdom';
            ...
            {{-- Form global untuk hapus Skdom (akan diisi otomatis oleh komponen) --}}
            <form id="formHapusSkdom"
                method="POST"
                data-action-template="{{ route('skdom.hapus', ['id' => '__ID__']) }}">
                @csrf
                {{-- hidden "id" akan dibuat/diisi otomatis oleh komponen saat modal dibuka --}}
            </form>
            <x-btn-hapus
                modalId="modalHapusSkdom"
                formId="formHapusSkdom"
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                const STORAGE_KEY = 'lastOpenedTabSkdom';
        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                const modalEl = document.getElementById('modalHapusSkdom');
                const modal   = new bootstrap.Modal(modalEl);
                const form    = document.getElementById('formHapusSkdom');
                const submit  = document.getElementById('btnHapusSkdom');
