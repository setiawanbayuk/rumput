        ...
        <div class="tab-content mt-3">
            <div class="tab-pane fade show active" id="tab-detail" role="tabpanel">
                ...
                <div class="text-center">
                    <a href="{{ route('skkematian.create') }}" class="btn-ajukan">
                        Ajukan
                    </a>
                </div>
            </div>
                ...
                <div class="container rounded-3 px-4 py-2" style="background:rgba(174,160,122,.25)">
                    @forelse($sedangProses as $pengajuan)
                        @php
                            ...
                            $alias  = $pengajuan->jenisSurat ?? 'skkematian';
            ...
            {{-- Form global untuk hapus Skkematian (akan diisi otomatis oleh komponen) --}}
            <form id="formHapusSkkematian"
                method="POST"
                data-action-template="{{ route('skkematian.hapus', ['id' => '__ID__']) }}">
                @csrf
                {{-- hidden "id" akan dibuat/diisi otomatis oleh komponen saat modal dibuka --}}
            </form>
            <x-btn-hapus
                modalId="modalHapusSkkematian"
                formId="formHapusSkkematian"
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                const STORAGE_KEY = 'lastOpenedTabSkkematian';
        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                const modalEl = document.getElementById('modalHapusSkkematian');
                const modal   = new bootstrap.Modal(modalEl);
                const form    = document.getElementById('formHapusSkkematian');
                const submit  = document.getElementById('btnHapusSkkematian');
